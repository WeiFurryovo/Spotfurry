...
│       ├── java/com/weifurry/spotfurry/data/auth
│       ├── java/com/weifurry/spotfurry/data/spotify
...
- 文件位置：`app/signing/spotfurry-debug.keystore`
- 用途：只用于 `debug APK` 和日常真机/模拟器测试
- 好处：本地构建和 GitHub Actions 产物签名一致，可以直接覆盖安装，不需要每次先卸载旧版本

这个 keystore 是公开测试签名，不是 release 签名。不要把正式发布签名、上架签名或任何私有 keystore 提交到仓库。
...
- 扫码入口：未配置有效 token 时，`Spotify WebView 实验` 会自动显示二维码
- 默认扫码后端：`https://spotfurry-auth.weifurry-c80.workers.dev`
...
spotfurry.spotifyAuthBaseUrl=https://你的 SpotfurryAuth Worker
```
...
./gradlew :app:assembleDebug \
  ...
  -Pspotfurry.spotifyAuthBaseUrl=https://你的 SpotifyAuth Worker
```
...
- 如果没有配置 `spotifyWebPlaybackAccessToken`，手表会通过 `spotifyAuthBaseUrl` 创建扫码会话，让手机登录 Spotify 后把短期 access token 发回手表。
...
Spotify 扫码登录后端放在独立仓库 `SpotfurryAuth`，当前 Android 主仓只保留手表端客户端逻辑，避免把 Worker 部署物和 App 混在一起维护。

后端需要提供这些接口：

- 创建会话：`POST /api/spotify/pairing/start`
- 手表轮询：`GET /api/spotify/pairing/status?sessionId=...`
- 手机授权页：`GET /spotify/pair?s=...&p=...&code=...`
- Spotify 登录跳转：`GET /spotify/login?s=...&p=...`
- Spotify 回调：`GET /spotify/callback`

部署前需要在 Spotify Developer Dashboard 创建 App，并添加 Redirect URI：

```text
https://你的 Worker 域名/spotify/callback
```

然后在 `SpotfurryAuth` 的 Worker secrets 中配置：

```text
SPOTIFY_CLIENT_ID=你的 Spotify Client ID
SPOTIFY_CLIENT_SECRET=你的 Spotify Client Secret
SPOTIFY_REDIRECT_URI=https://你的 Worker 域名/spotify/callback
```

本地不要提交 Spotify Client Secret、access token、refresh token 或任何真实账号配置。
