...
spotfurry.spotifyAuthBaseUrl=https://你的 SpotifyAuth Worker
```
...
仓库包含一个 Cloudflare Worker 示例后端：

- 路径：`workers/spotify-auth-worker/`
- 创建会话：`POST /api/spotify/pairing/start`
- 手表轮询：`GET /api/spotify/pairing/status?sessionId=...`
- 手机授权页：`GET /spotify/pair?sessionId=...&code=...`
- Spotify 回调：`GET /spotify/callback`

部署前需要：

1. 在 Spotify Developer Dashboard 创建 App。
2. 添加 Redirect URI：`https://你的 Worker 域名/spotify/callback`。
3. 创建 Cloudflare KV namespace，并把 ID 填到 `workers/spotify-auth-worker/wrangler.toml.example` 复制出的 `wrangler.toml`。
4. 在 Worker 环境变量或 secrets 中配置：
...
本地不要提交 `wrangler.toml` 里的真实 KV ID、Spotify Client Secret、access token 或 refresh token。
