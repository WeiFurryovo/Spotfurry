...
spotfurry.appleMusicPairingBaseUrl=https://你的配对后端.example/apple-music/pair
```
...
./gradlew :app:assembleDebug \
  ...
  -Pspotfurry.appleMusicPairingBaseUrl=...
```
...
- 扫码登录当前只负责在手表上显示配对二维码；真正的手机授权页、短码校验、token 交换和手表轮询需要由后端实现。
- `spotfurry.appleMusicPairingBaseUrl` 必须是手机能访问到的真实 `https://` 地址，不能使用 `.invalid`、内网不可达地址或未部署的域名。
...
Spotify 保留为后续可选方向。公开 API 下更适合做 `Web API 数据读取 + Spotify Connect 控制`，如果要尝试在 Spotfurry 内直接响 Spotify 音乐，只建议做 `WebView + Web Playback SDK` 实验分支。
